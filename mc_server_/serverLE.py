...
def init_db():
	im.execute("CREATE TABLE IF NOT EXISTS liste\
	(telefon PRIMARY KEY,isim,mail,okul,bolum,sinif,uid,sayac INTEGER,kontrol INTEGER)")
	db.commit()
...
def uid_artir(*args):
	...
	if sorgu==None:
		k_ö = str(uuid.uuid1())
		im.execute("INSERT INTO liste VALUES (?,'k.ö','k.ö','k.ö','k.ö',\
										'k.ö',?,?,?)",(k_ö,args[0],1,1))							
		db.commit()
		return 'Geçersiz kayit!'
	elif sorgu[1]==0:
		im.execute("UPDATE liste SET sayac = ?, kontrol = 1\
							WHERE uid = ?",(sorgu[0]+1,args[0]))		
		db.commit()
